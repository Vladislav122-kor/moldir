...
class PaymentAndDeliveryContainer extends Component {
    ...
    private createTextForInformBlock() {
        ...
        const blocks = [
            'Заказать товар можно по номеру телефона <b>+375 44 505 39 49</b> (звонок/Viber), через Instagram, или написав на почту moldir.minsk@mail.ru.<br>Раздел "Корзина" временно не работает, приносим извинения за доставленные неудобства.',
            'Выбирая товар <b>в наличии</b>, вы можете оплатить его при получении либо посредством онлайн-оплаты.<br>Если товар есть в наличии, доставка возможна в этот же день, при условии оформления заказа до 15:00. В иных случаях доставка будет осуществляться на следующий день.',
            'Если товар помечен надписью <b>"под заказ"</b>, его доставка осуществляется по предоплате 70%. Предоплата производится онлайн после подтверждения заказа оператором. В таком случае товар доставляется в течение 10 рабочих дней.',
            'Доставка по Минску и в пределах 10 км от МКАД осуществляется бесплатно. Доставка за пределы 10 км от МКАД согласовывается индивидуально.<br>Время доставки ежедневно с 18:00 до 22:00.'
        ];
